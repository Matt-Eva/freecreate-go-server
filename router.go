...
func CreateRouter(sessionStore *sessions.CookieStore, pgxPools config.PgxPools, pgCoreQueries config.PgCoreQueries, valkeyClient valkey.Client, resendClient *resend.Client) *chi.Mux {
	...
	router.Get("/login", web_page_handlers.LoginPageHandler(sessionStore, templates, pgxPools.PgCore, pgCoreQueries))
	...
	router.Get("/about", web_page_handlers.AboutPageHandler(templates, sessionStore))
